...
describe('Visualization space 24 hour chart', function() {
  ...
  describe('data fetching', function() {
    it('should fetch data and display it', async function() {
          ...
          results: [
            {
              count: 0,
              interval: {
                start: "2017-01-1T03:55:00.000Z",
                end: "2017-01-1T03:59:59.999Z",
                analytics: {min: 0, max: 0},
              },
              timestamp: "2017-01-1T03:55:00.000Z",
            },
            {
              count: 1,
              interval: {
                start: "2017-01-1T04:00:00.000Z",
                end: "2017-01-1T04:05:00.000Z",
                analytics: {min: 0, max: 1},
              },
              timestamp: "2017-01-1T04:00:00.000Z",
            },
            {
              count: 2,
              interval: {
                start: "2017-01-1T04:05:00.000Z",
                end: "2017-01-1T04:10:00.000Z",
      ...
      assert.equal(requestParameters[1].qs.start_time, '2017-01-01T05:00:00Z');
    ...
    it('should fetch data and display it during a different part of the year', async function() {
      ...
      const hoursOffsetFromUtc = parseInt(moment.tz(space.timeZone).format('Z').split(':')[0], 10);
      assert.equal(requestParameters[1].qs.start_time, '2017-09-14T04:00:00Z');
    ...
    it('should fetch data and display it in a different time zone', async function() {
      ...
      // On January 1st, the offset is NYC is 5 hours.
      const hoursOffsetFromUtc = parseInt(moment.tz(space.timeZone).format('Z').split(':')[0], 10);
      assert.equal(requestParameters[1].qs.start_time, '2017-01-01T08:00:00Z');
      assert.equal(requestParameters[1].qs.end_time, '2017-01-02T08:00:00Z');
