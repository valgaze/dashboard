...
function calculatePeakRateOfEntry(space, minutelyBucketsByDay) {
  ...
  const peakRateOfEntryPerDayTimestamp: number[] = [];
  const peakRateOfEntryPerDayValue: number[] = [];
...
export default async function averageTimeSegmentBreakdown(report) {
  ...
  const timeSegmentGroup: any = objectSnakeToCamel(await core.time_segment_groups.get({
  ...
  const minutelyBucketsByDay = minutelyCounts.reduce((acc, bucket: any) => {
